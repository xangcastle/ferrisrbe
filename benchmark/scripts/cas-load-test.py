...
def upload_blob_batch(
    ...
    try:
        ...
        response = cas_stub.BatchUpdateBlobs(request, metadata=metadata or [])
...
def download_blob_batch(
    ...
    try:
        ...
        response = cas_stub.BatchReadBlobs(request, metadata=metadata or [])
...
def run_load_test(
    ...
    with ThreadPoolExecutor(max_workers=concurrent) as executor:
        futures = {
            executor.submit(upload_blob_batch, cas_stub, data, hash_hex, grpc_metadata): (data, hash_hex)
    ...
    with ThreadPoolExecutor(max_workers=concurrent) as executor:
        futures = {
            executor.submit(download_blob_batch, cas_stub, hash_hex, blob_size, grpc_metadata): hash_hex
