...
fn main() -> Result<()> {
    let worker_proto = resolve_proto_file("proto/worker.proto")
        .expect("worker.proto not found");
    let remote_execution_proto = resolve_proto_file("proto/build/bazel/remote/execution/v2/remote_execution.proto")
    ...
    let status_proto = resolve_proto_file("proto/google/rpc/status.proto")
        .expect("status.proto not found");
    ...
    let any_proto = resolve_proto_file("proto/google/protobuf/any.proto")
        .expect("any.proto not found");
    let annotations_proto = resolve_proto_file("proto/google/api/annotations.proto")
        .expect("annotations.proto not found");
    let http_proto = resolve_proto_file("proto/google/api/http.proto")
        .expect("http.proto not found");
    
    let proto_include = worker_proto.parent()
